...
chars = chars ? _half2full(chars, { punctuation: true, smart_mode: false }) : "上海纽约大学字圆其说美哉汉乐开书店内里百货";
res = res ? res : 48;
scl = scl ? scl : 2;
clr = clr ? clr : 0;
dev = dev ? dev : 0;
  ...
  if (!DetectRTC.isWebRTCSupported) {
    if (DetectRTC.isMobileDevice) {
      if (DetectRTC.osName == "iOS") {
        if (parseInt(DetectRTC.osVersion.split(".")[0]) < 11) {
          // iOS needs to be 11+
...
if (rf == 1) {
  ...
  // display toolbar when not rf is off (not in exhibt mode)
  toggleToolbar();
}
